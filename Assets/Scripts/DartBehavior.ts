...
export class DartBehavior extends BaseScriptComponent {
  ...
  onTriggerStart(e: InteractorEvent) {
    ...
    this.hand = this.handInputData.getHand(
      inputType == InteractorInputType.LeftHand ? "left" : "right"
    );

    let startPoint = this.hand.indexKnuckle.position
      .add(this.hand.thumbKnuckle.position)
      .uniformScale(0.5);
    let nudgeLeftDir = this.hand.middleKnuckle.position.sub(
      this.hand.pinkyKnuckle.position
    );
    startPoint = startPoint.add(nudgeLeftDir.normalize().uniformScale(5));
    let nudgeUpDir = this.hand.indexKnuckle.position.sub(
      this.hand.wrist.position
    );
    startPoint = startPoint.add(nudgeUpDir.normalize().uniformScale(3));

    let endPoint = this.hand.indexTip.position
      .add(this.hand.thumbTip.position)
      .uniformScale(0.5);
  ...
  onTriggerEnd() {
    ...
    baseVelocity = baseVelocity.uniformScale(
      this.HAND_BASE_VELOCITY_MULTIPLIER
    );
  ...
  onCollisionEnter(e) {
    ...
    if (isDartBoardHit) {
      ...
      if (this.t.getWorldPosition().distance(this.prevPos) > 1.2) {
        let res = this.getTouchPointPlaneIntersections(
          touchPoint,
          touchRotation
        );
        touchPoint = res[0];
      ...
      if (this.isStraightHit(touchRotation)) {
        ...
        let parentWorldPosition = childWorldPosition.sub(
          touchRotation.multiplyVec3(
            childLocalPosition.mult(parentTransform.getWorldScale())
          )
        );
  ...
  isStraightHit(rot: quat) {
    let angle =
      this.dartBoardT.forward.angleTo(rot.multiplyVec3(vec3.forward())) *
      (180 / Math.PI);
  ...
  onUpdate() {
    if (this.isHolding && getDeltaTime() > 0) {
      let handAcceleration = this.getHandVelocity()
        .sub(this.prevHandVelocity)
        .uniformScale(1 / Math.max(0.016666, getDeltaTime()));
      this.accumulatedForce = this.accumulatedForce.add(
        handAcceleration.uniformScale(this.HAND_ACCELERATION_MULTIPLIER)
      );
  ...
  getTouchPointPlaneIntersections(curPoint, curRot): [vec3, quat] {
    ...
    let negativeDist = -this.dartBoardT
      .getWorldPosition()
      .sub(curPoint)
      .dot(this.dartBoardT.forward);
    ...
    if (negativePoint != null && positivePoint != null) {
      let p = this.getPlaneIntersectionsBetweenPoints(
        negativePoint,
        positivePoint
      );
      return [p, curRot];
  ...
  getPlaneIntersectionsBetweenPoints(pointA, pointB) {
    ...
    let dist =
      this.dartBoardT
        .getWorldPosition()
        .sub(pointA)
        .dot(this.dartBoardT.forward) + this.BOARD_OFFSET;
  ...
  getHandVelocity(): vec3 {
    ...
    if (global.deviceInfoSystem.isEditor()) {
      return WorldCameraFinderProvider.getInstance()
        .forward()
        .uniformScale(-1050);
    }
