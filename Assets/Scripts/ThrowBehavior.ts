import { HandInteractor } from "SpectaclesSyncKit/SpectaclesInteractionKit/Core/HandInteractor/HandInteractor";
import {
  Interactor,
  InteractorTriggerType,
} from "SpectaclesSyncKit/SpectaclesInteractionKit/Core/Interactor/Interactor";
import { MouseInteractor } from "SpectaclesSyncKit/SpectaclesInteractionKit/Core/MouseInteractor/MouseInteractor";
import { SIK } from "SpectaclesSyncKit/SpectaclesInteractionKit/SIK";
import { mix } from "SpectaclesSyncKit/SpectaclesInteractionKit/Utils/animate";
...
export class ThrowBehavior extends BaseScriptComponent {
  ...
  onUpdate() {
    if (this.rightHand.isPinching()) {
      this.latestBall
        .getTransform()
        .setWorldPosition(this.rightHand.indexTip.position);
    }
  ...
  onTriggerDown() {
    ...
    this.latestBall.getComponent("MeshVisual").mainMaterial = this.latestBall
      .getComponent("MeshVisual")
      .mainMaterial.clone();
    this.latestBall.getComponent("MeshVisual").mainMaterial.mainPass.mainColor =
  ...
  onTriggerUp() {
    this.debugText.text =
      "Force: " + this.magnitude(this.getHandVelocity(1, 0)).toFixed(1);
  ...
  getHandVelocity(globalAmount, indexAmount) {
    ...
    if (objectSpecificData) {
      ...
      let averagedVelocity = handVelocity
        .uniformScale(globalAmount)
        .add(fingerVelocity.uniformScale(indexAmount));
